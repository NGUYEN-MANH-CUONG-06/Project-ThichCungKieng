...
export function RitualCatalog() {
   ...
   * - Function này là dependency của useEffect phía dưới
   * - Không có useCallback → function tạo mới mỗi render → useEffect chạy liên tục
   * - Có useCallback → function chỉ tạo lại khi deps thay đổi (searchParams, setSearchParams)
   *
   * LOGIC FUNCTION:
   * - Tạo URLSearchParams mới từ params hiện tại
   * - Set/delete param "search" tuỳ value
   * - Reset về page 1 (khi search mới thì xem từ đầu)
   * - Update URL → trigger re-render + API call
   */
   ...
   *
   * CHỈ DEPS `debouncedSearch`:
   * - Chỉ chạy khi `debouncedSearch` thay đổi (500ms sau khi user gõ xong)
   * - Không deps `updateSearchParam` vì đã memoize bằng useCallback
   * - Không deps `searchParams.get("search")` vì gây infinite loop
   *
   * eslint-disable: Báo ESLint bỏ qua warning thiếu deps.
   * Lý do: Thêm `updateSearchParam` vào deps → vòng lặp vô hạn:
   *   debouncedSearch → update URL → searchParams change → updateSearchParam mới
   *   → useEffect chạy lại → loop ♾️
   */
  useEffect(() => {
